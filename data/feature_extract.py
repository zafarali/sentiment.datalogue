...
for n_gram in N_GRAM:
	for n_features in N_FEATURES:
		...
		hv = HashingVectorizer(stop_words=STOPWORDS, n_features=n_features, ngram_range=n_gram)
		hv.fit(CORPUS)

		# create a pipeline for tfidf calculations
		hv2 = HashingVectorizer(stop_words=STOPWORDS, n_features=n_features, norm=None, ngram_range=n_gram)
		tf = TfidfTransformer()
